...
def save_log_db(project_name, experiment_index, epoch, log):
    log_db = TrainLog(project_name=project_name, experiment_index=experiment_index, epoch=epoch, log=log)
...
def make_fake_db():
    ### create the fixtures 
    save_project_db('interojo', 'wonchul', 'abc')
    save_project_db('central', 'wonchul2', 'edf')
    
    save_experiment_db("interojo", {"datasets": "interojo_ver2", "input_dir": 'abcdefghi'}, {"a": 1, "b": 2, "c": "abc"})
    save_experiment_db("interojo", {"datasets": "interojo_ver22", "input_dir": 'ii'}, {"a": 100, "b": 20, "c": "abc"})

    save_log_db("interojo", 0, 0, {"epoch": 0, "train_loss": 0.3, "val_loss": 0.2, "lr": 0.01})
    save_log_db("interojo", 0, 1, {"epoch": 1, "train_loss": 0.2, "val_loss": 0.1, "lr": 0.001})
